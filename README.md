...
`/v1/memory` is the NullClaw CLI-shaped command surface over the same actor-aware core: `GET /v1/memory/count`, `GET /v1/memory/list`, `POST /v1/memory/search`, `GET /v1/memory/get/:key`, `POST|PUT /v1/memory/store/:key`, `POST|PUT /v1/memory/update/:key`, `DELETE|POST /v1/memory/delete/:key`, `DELETE|POST /v1/memory/forget/:key`, `POST /v1/memory/export-jsonl`, and `POST /v1/memory/hygiene-report`. It accepts `session` as an alias for `session_id`, defaults CLI-style stores to category `conversation`, keeps `update` strict by returning `memory_not_found` instead of creating a missing row, returns `deleted:false` for missing deletes, and still supports scoped/shared memory plus storage selectors (`native`, `runtime`, named stores, subsets, `all`). The native feed/apply endpoints remain under `/v1/memory/feed`, `/v1/memory/events`, `/v1/memory/status`, `/v1/memory/compact`, `/v1/memory/checkpoint`, and `/v1/memory/apply`.
