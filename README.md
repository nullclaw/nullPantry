...
- `GET /v1/engines` exposes the NullClaw memory engine registry and the truth about each integration mode. `sqlite`, `postgres`, and `kg` are remote-primary NullPantry storage strategies; `markdown`, `memory_lru`, `lucid`, `redis`, `clickhouse`, and `lancedb` are represented as import/cache/vector/export adapter contracts rather than byte-for-byte NullClaw local engine clones.
