...
```sh
export NULLPANTRY_EMBEDDING_PROVIDER="openai-compatible" # openai-compatible | gemini | voyage | ollama | local-deterministic
export NULLPANTRY_EMBEDDING_FALLBACKS='[{"provider":"voyage","api_key":"..."},{"provider":"ollama","base_url":"http://localhost:11434"},{"provider":"local-deterministic"}]'
...
Provider endpoints and API keys are server-side configuration only; request bodies cannot override provider `provider`, `base_url`, `api_key`, model, timeout, or insecure-HTTP fields. Provider URLs must be HTTPS or local loopback HTTP by default; non-local plain HTTP requires explicit opt-in with `NULLPANTRY_PROVIDER_ALLOW_INSECURE_HTTP=true`, `NULLPANTRY_EMBEDDING_ALLOW_INSECURE_HTTP=true`, `NULLPANTRY_LLM_ALLOW_INSECURE_HTTP=true`, `--provider-allow-insecure-http`, `--embedding-allow-insecure-http`, `--llm-allow-insecure-http`, or per-fallback JSON `{"allow_insecure_http":true}`. Embeddings are first-class for OpenAI-compatible APIs, Gemini `embedContent`, Voyage `/v1/embeddings`, native Ollama `/api/embed`, and local deterministic fallback. `NULLPANTRY_EMBEDDING_FALLBACKS` accepts a comma-separated provider list or JSON endpoint objects and is tried server-side after the primary provider fails. Provider calls are protected by a shared runtime circuit breaker with configurable `NULLPANTRY_PROVIDER_CIRCUIT_FAILURE_THRESHOLD` and `NULLPANTRY_PROVIDER_CIRCUIT_COOLDOWN_MS`; diagnostics expose closed/open/half-open state, skips, failures, and successes without leaking API keys. If no embedding provider is configured, NullPantry uses deterministic local embeddings. Explicit embedding/indexing operations fail closed when the configured provider fails and no fallback succeeds. Retrieval endpoints degrade to permission-filtered keyword/global/entity search by default so `/v1/search`, `/v1/ask`, and `/v1/context-packs` remain usable during provider outages; pass `"strict_vector": true` to fail the request instead. `/v1/ask` remains extractive by default even when an LLM provider is configured; callers must pass `"use_llm": true` to send retrieved evidence to the provider. `GET /v1/providers` exposes the concrete and compatible provider contracts for agents and deployment tooling.
...
`/v1/memory` is the NullClaw CLI-shaped command surface over the same actor-aware core: `GET /v1/memory/count`, `GET /v1/memory/list`, `POST /v1/memory/search`, `GET /v1/memory/get/:key`, `POST|PUT /v1/memory/store/:key`, `POST|PUT /v1/memory/update/:key`, `DELETE|POST /v1/memory/delete/:key`, `DELETE|POST /v1/memory/forget/:key`, `POST /v1/memory/export-jsonl`, and `POST /v1/memory/hygiene-report`. It accepts `session` as an alias for `session_id`, supports `include_internal` and `include_content=false` for metadata-only list/search responses, defaults CLI-style stores to category `conversation`, keeps `update` strict by returning `memory_not_found` instead of creating a missing row, returns `deleted:false` for missing deletes, treats omitted `session` on delete as exact global-memory deletion, exposes explicit all-session deletion through `all_sessions=true` or `delete_all=true`, and still supports scoped/shared memory plus storage selectors (`native`, `runtime`, named stores, subsets, `all`). The native feed/apply endpoints remain under `/v1/memory/feed`, `/v1/memory/events`, `/v1/memory/status`, `/v1/memory/compact`, `/v1/memory/checkpoint`, and `/v1/memory/apply`.

For the current NullClaw `api` memory engine, NullPantry also exposes an adapter namespace at `/v1/agent`: `PUT|GET|DELETE /v1/agent/memories/:key`, `GET /v1/agent/memories`, `POST /v1/agent/memories/search`, `GET /v1/agent/memories/count`, `GET|POST|DELETE /v1/agent/sessions/:id/messages`, `GET|PUT|DELETE /v1/agent/sessions/:id/usage`, `DELETE /v1/agent/sessions/:id`, `POST /v1/agent/sessions/:id/terminate`, `DELETE /v1/agent/sessions/auto-saved?session_id=:id`, `GET /v1/agent/history`, `GET /v1/agent/history/:id`, `GET /v1/agent/memory/events`, `GET /v1/agent/memory/status`, `GET /v1/agent/memory/stats`, `POST /v1/agent/memory/reindex`, `POST /v1/agent/memory/drain-outbox`, `POST /v1/agent/memory/compact`, `GET|POST /v1/agent/memory/checkpoint`, and `POST /v1/agent/memory/apply`. The `/v1/agent/memory/*` feed is a NullClaw `ApiMemory`-compatible projection over NullPantry's central event log: it emits only replayable `agent_memory` events, includes top-level `key`, `timestamp_ms`, `schema_version`, `value_kind`, and `content` fields, accepts top-level NullClaw feed events, keeps origin dedupe idempotent, and restores checkpoints through the native actor-aware core. This is a thin contract adapter over the same actor-aware NullPantry core, so a NullClaw config can use `url=http://host:8765` with `namespace=/v1/agent` without a sidecar shim.

NullClaw prompt bootstrap files are first-class agent memory in NullPantry. `GET /v1/bootstrap/prompts` lists the registered prompt documents (`AGENTS.md`, `SOUL.md`, `TOOLS.md`, `CONFIG.md`, `IDENTITY.md`, `USER.md`, `HEARTBEAT.md`, `BOOTSTRAP.md`, `MEMORY.md`), `PUT|GET|DELETE /v1/bootstrap/prompts/:filename` manages the corresponding `__bootstrap.prompt.*` memory entry, `GET /v1/bootstrap/prompts/:filename/exists` checks visibility, `GET /v1/bootstrap/prompts/:filename?max_bytes=N` returns a bounded excerpt, `GET /v1/bootstrap/prompts/fingerprint` returns a stable provider fingerprint, and `POST /v1/bootstrap/prompts/import-directory` hydrates all known files from a workspace directory into actor-private or scoped shared NullPantry memory.

Agent-memory calls can choose the storage plane per request. Body fields `storage`, `store`, `target_store`, or exact array `stores`, and query params `?storage=...`, `?store=...`, `?target_store=...`, or comma-separated `?stores=scratch,archive`, accept `primary`, `native`, `redis`/`runtime`, any configured named store id, and `all`/`federated`. `primary` preserves the configured default plane: Redis or memory_lru runtime when `NULLPANTRY_AGENT_MEMORY_BACKEND` is configured, otherwise canonical SQLite/Postgres. `native` forces canonical SQLite/Postgres, `redis`/`runtime` forces the default runtime plane and returns `storage_unavailable` if it is not configured, a named store routes only to that runtime store, `stores:["scratch","archive"]` writes/reads only those selected stores, and `all` writes/deletes/federates canonical native, default runtime, and every named runtime with runtime results preferred over native duplicates. Federated `/v1/search` keeps same-key agent memories from different stores distinct and includes `store`/`storage` on those results. Cross-plane writes are intentionally not a distributed transaction; canonical storage remains the system of record for provenance, ACL, lifecycle, and retrieval.
...
`GET /v1/agent-sessions`, `GET|DELETE /v1/agent-sessions/:id`, `POST /v1/agent-sessions/:id/terminate`, `GET|POST|DELETE /v1/agent-sessions/:id/messages`, `GET|PUT|DELETE /v1/agent-sessions/:id/usage`, and `DELETE /v1/agent-sessions/auto-saved?session_id=:id` are the native session/history API. Multiple agents may reuse the same `session_id`; results are always filtered by token-bound `actor_id` before session scope checks. Termination mirrors NullClaw `agent sessions terminate`: it clears the caller-visible session messages, autosave rows, session usage, and session-scoped agent memory entries in the selected storage plane while preserving other actors' state for the same session id.
...
- `POST /v1/ingest` and `POST /v1/extract-memory` are durable-first: they create a source plus queued job by default, and workers derive artifacts, extract structured memory lines such as `Decision:`, `Constraint:`, `Action:`, `Question:`, `Risk:`, ticket fields, PR/commit links, dependencies, incident symptoms, root causes, and mitigations, resolve Null ecosystem entities, and apply the extracted artifact/entities/relations/atoms as one storage operation before indexing rebuildable vector chunks. Pass `run_now:true` only for tests or controlled one-shot imports.
- `POST /v1/connectors/:name/ingest` and `GET|POST /v1/connectors/:name/cursor` provide built-in push connector contracts for NullTickets, NullWatch, tickets, incidents, transcripts, git/PR imports, QMD JSON search results, and manual sources. Connector cursors are permission-filtered first-class records, so sync state cannot leak across projects/teams, and connector ingests advance `next_cursor` only after every submitted item has imported successfully. The `qmd` connector accepts `results`/`items` shaped like QMD `--json` output (`path`, `title`, `content`/`text`, `start_line`, `end_line`), normalizes them into canonical Sources with provenance metadata, and then uses the same extraction/vector/lifecycle path as every other source.
...
- Transcript extraction accepts timestamp/speaker-prefixed lines such as `[00:01:04] Alice: Decision: ...`, records evidence ranges, and chunks long sources, artifacts, extracted entities, relations, and memory atoms into multiple permission-filtered vector chunks instead of indexing only one whole-document vector.
- `GET|POST /v1/jobs`, `POST /v1/jobs/:id/run`, and `POST /v1/workers/run` persist and execute extraction, hygiene, conflict scan, and vector-outbox jobs with scoped visibility. Hygiene jobs honor their `input` lifecycle options such as `stale_after_ms`, `archive_after_ms`, `purge_after_ms`, `hard_delete`, and `dedupe_memory_atoms`, but worker execution is narrowed to the job's own scope/permissions even when the service worker has an admin principal.
...
- `GET /v1/graph/schema`, `POST /v1/graph/query`, `POST /v1/graph/neighbors`, and `POST /v1/graph/path` expose the native knowledge graph. Relation types are canonicalized through a built-in ontology (`depends_on`, `implements`, `produced`, `supersedes`, `documents`, `affected`, `fixes`, `belongs_to`, `owns`, `used_context_pack`, plus `related_to`), traversal supports `direction`, `relation_types`, `entity_types`, `min_confidence`, `depth`, and `include_deprecated`, and every hop re-checks relation ACL, endpoint entity ACL, source citation visibility, and lifecycle status. `POST /v1/graph/query`, `POST /v1/graph/path`, `/v1/search`, `/v1/retrieval/search`, `/v1/ask`, `/v1/context-packs`, `/v1/agent-memory/search`, and `/v1/agent/memories/search` also accept compact graph commands (`kg:traverse:{entity}:{depth}`, `kg:path:{from}:{to}:{depth}`, `kg:relations:{entity}`, with percent-encoded identifiers) that execute against the same native Entity/Relation model. Agent-memory graph-command responses are synthetic read-only entries backed by the graph and are not materialized into agent memory stores. `PATCH|DELETE /v1/entities/:id` and `PATCH|DELETE /v1/relations/:id` provide lifecycle mutations for graph primitives instead of physical deletion.
- `GET /v1/engines` exposes runtime planes instead of a flat compatibility list: record storage, agent memory/session storage, vector, graph, projection, analytics/audit export, import/export, lifecycle, and feed. Descriptors include NullClaw-style discovery flags (`nullclaw_engine_token`, `auto_save_default`, `capabilities.keyword_rank/session_store/transactions/outbox`, and `requirements.db_path/workspace`) so migration tools can choose a backend without hard-coded tables. `sqlite` and `postgres` remain full system-of-record backends; `hybrid` is the governed SQLite/Postgres plus Markdown import/export composition for NullClaw hybrid compatibility without making Markdown an ungoverned primary; `none`, `memory_lru` (aliases `memory`, `in_memory`), and `redis` are runtime backends for disabled, in-process, and shared/isolated agent memory, session messages, and usage state; `qmd` is a governed import/retrieval-source compatibility plane for QMD markdown/session result JSON; `kg` is native graph retrieval; `pgvector` is the Postgres-coupled vector plane; `qdrant` and `lancedb` are external runtime vector planes; `clickhouse` is the runtime analytics/event-history export plane; `lucid` is an optional durable CLI projection plane for semantic context.
- `GET /v1/vector/status`, `POST /v1/vector/embed`, `POST /v1/vector/upsert`, `POST /v1/vector/search`, `POST /v1/vector/delete`, `POST /v1/vector/rebuild`, `POST /v1/vector/reconcile`, `GET /v1/vector/outbox`, and `POST /v1/vector/outbox/run` provide the server-side vector layer with deterministic local embeddings, OpenAI-compatible/Gemini/Voyage/Ollama embedding providers, heading/paragraph-aware UTF-8-safe chunking, type-qualified vector chunk IDs, permission-filtered vector chunks for sources, artifacts, memory atoms, entities, relations, persisted context packs, native agent memory, spaces, and policy scopes, ANN-style prefiltering, final cosine rerank, local `indexed_local` outbox acknowledgements, dynamic pgvector dimensions in Postgres, Qdrant HTTP upsert/search/delete/reset, LanceDB SDK upsert/search/delete/reset, and an explicit LanceDB HTTP adapter-service contract. Status and diagnostics distinguish the configured vector sink (`local`, `qdrant`, `lancedb`, `lancedb_http`) from the local canonical search engine (`sqlite_local_vector` or `pgvector`) and the effective search engine used for retrieval. External vector search returns candidate vector IDs only; NullPantry hydrates those IDs from its canonical store and re-applies ACL/status checks before returning text. Delete emits external tombstones, reconcile replays canonical chunks into the configured vector plane, and rebuild can reset the external collection/table before replaying canonical truth. Direct `/v1/vector/search` accepts `strict_external`/`strict_vector` to fail closed when the configured external vector plane is unavailable; retrieval endpoints keep the existing graceful fallback unless strict vector retrieval is requested.
- `POST /v1/retrieval/plan` exposes retrieval planning primitives for keyword, vector, graph, query expansion, MMR/RRF, minimum relevance filtering, temporal decay, adaptive retrieval, and reranking. Query expansion returns the expanded vector/keyword query, the Postgres `websearch` query form, expansion terms, expansion reasons, and intent hints so agents can inspect why retrieval changed the original query.
- `/v1/search`, `/v1/ask`, `/v1/context-packs`, and `POST /v1/retrieval/search` use the same hybrid retrieval path: ACL first, scoped keyword/global candidates, deterministic query expansion for document/memory retrieval, exact original-query entity/relation lookup before graph expansion, configured query embeddings, vector ANN search, optional Lucid projection augmentation when local hits are sparse, RRF fusion, `min_relevance`/`min_score` filtering, temporal decay rerank, embedding MMR for local-deterministic embeddings with token-diversity fallback for external embedding spaces, optional provider-backed LLM rerank with stable candidate IDs when `allow_reranker=true`, citation-safe result assembly, staleness warnings, optional visible conflict warnings, and grouped results for memory atoms, sources, artifacts, entities, relations, context packs, feed events, native agent memories, Lucid projections, and guarded session messages. Context packs return both the generated text summary and typed sections/citations/forbidden assumptions/suggested next steps for agent context serving. Read-only callers receive non-persisted context pack previews; durable context packs require `write` or `propose` and can be requested with `"persist": true`. Ask lists existing visible conflicts by default; callers with `write` or `verify` can pass `"scan_conflicts": true` to update conflict records during the request.
- `GET|POST /v1/memory/feed`, `GET|POST /v1/memory/events`, `GET /v1/memory/status`, `GET /v1/memory/count`, `GET /v1/memory/list`, `POST /v1/memory/search`, `GET /v1/memory/get/:key`, `POST|PUT /v1/memory/store/:key`, `POST|PUT /v1/memory/update/:key`, `DELETE|POST /v1/memory/delete/:key`, `DELETE|POST /v1/memory/forget/:key`, `POST /v1/memory/export-jsonl`, `POST /v1/memory/hygiene-report`, `POST /v1/memory/reindex`, `POST /v1/memory/drain-outbox`, `POST /v1/memory/compact`, `GET|POST /v1/memory/checkpoint`, and `POST /v1/memory/apply` provide native cross-memory feed/apply events plus NullClaw-compatible memory command and maintenance aliases. `GET /v1/agent/memory/stats`, `POST /v1/agent/memory/reindex`, and `POST /v1/agent/memory/drain-outbox` are adapter aliases for agents that still speak the NullClaw API shape; internally they delegate to the same governed stats/vector rebuild/outbox worker. Feed visibility, pending restore, and apply use the same scope plus permissions checks as sources, artifacts, entities, relations, context packs, memory atoms, agent memory, and actor-isolated agent session messages/usage. Apply validates payloads before reserving dedupe keys, preserves the event `actor_id` for admin/service restores, preserves agent-memory and session storage planes from event payloads (`native`, default `runtime`, named stores, subsets, or `all`), supports `put`, `delete`, `verify`, `mark_stale`, `supersede`, deterministic `merge_string_set`, and deterministic `merge_object`, returns the original applied event on retry, releases stale in-progress reservations after the retry window, exports cursor-floor checkpoints, redacts payloads that cite inaccessible objects or another actor's session data, and returns `410 Gone` when a consumer asks for events below the compacted cursor floor. `NULLPANTRY_INSTANCE_ID` / `NULLPANTRY_FEED_INSTANCE_ID` / `--instance-id` sets the feed origin identity used in status, checkpoints, and synthetic replay dedupe keys, so checkpoints from multiple NullPantry instances do not collide.
- `GET /v1/lifecycle/diagnostics`, `GET /v1/lifecycle/stats`, `GET /v1/memory/stats`, `POST /v1/lifecycle/migrate`, `POST /v1/lifecycle/snapshot`, `POST /v1/lifecycle/snapshot/export`, `POST /v1/lifecycle/export-jsonl`, `POST /v1/lifecycle/snapshot/import`, `POST /v1/lifecycle/snapshot/hydrate`, `POST /v1/lifecycle/hydrate`, `GET /v1/lifecycle/analytics/status`, `POST /v1/lifecycle/analytics/query`, `POST /v1/lifecycle/analytics/export`, `POST /v1/lifecycle/hygiene-report`, `POST /v1/lifecycle/hygiene`, `POST /v1/lifecycle/summarize`, and `POST /v1/lifecycle/rollout` expose lifecycle runtime operations. Diagnostics include memory, stale memory, vector outbox, cache, queued/running/failed jobs, pending feed events, open conflicts, native agent memories, session counts, and a redacted runtime section for record backend health/schema, agent-memory backend capabilities, no-op status, named stores, vector/lucid/analytics planes, cache, retrieval capabilities, embedding provider, and completion provider configuration. Stats is the compact NullClaw-compatible runtime counter shape for agents and operators: it returns backend/retrieval/vector/embedding/rollout/sync/fallback plus permission-aware visible source and agent-memory counts, vector chunk count, pending vector outbox count, and a richer `nullpantry` block with record backend, provider, cache, jobs, feed, and conflict counters. Lifecycle migrate plans and executes ACL-checked, dry-run-first `agent_memory`, `agent_sessions`, and combined `all` movement between `native`, default runtime, named, subset, and `all` storage routes, with optional source deletion for real storage-plane cutovers. Combined migration preflights both memory and session state before writes. Session migration preserves message timestamps, copies usage, and requires `overwrite` or `append` if the target session already has state. Snapshot export is permission-aware and returns only visible search/context objects. `read + export` can produce a pure export without writing a lifecycle snapshot; persisted export snapshots require `write` or `propose`. JSONL export emits one visible object per line using a stable `nullpantry.memory_jsonl.v1` schema and redacts common PII/secrets by default; `include_pii:true` is required for raw trusted exports. Hygiene report is a read-only, permission-aware dry run over visible agent memory and memory atoms; it counts exact and normalized duplicate groups without mutating storage, respects storage selectors for agent memory, and excludes bootstrap/autosave internals unless `include_internal:true` is set. Lifecycle hygiene can also run an ACL-checked memory-atom dedupe pass with `dedupe_memory_atoms:true`, keeping the strongest/latest fact and deprecating or hard-deleting duplicates only when the caller has the matching verify/delete scope. Snapshot import/hydrate accepts either `objects` or a full exported `snapshot.objects` payload, supports `dry_run`/`preflight` validation without writes, and hydrates portable objects back as provenance-backed memory atoms through an atomic storage operation subject to the caller's write/propose/verify scopes. Summarize is a structured extractive summarizer by default: it emits bounded text plus typed decisions, constraints, action items, risks, open questions, facts, and message-index evidence; optional LLM summarization uses only server-configured providers and falls back to the structured summary unless `strict_llm:true` is set. Rollout evaluates `off`/`shadow`/`canary`/`on` policies with deterministic actor/session buckets, scope and capability gates, kill switches, and explicit reasons for agent orchestration. Analytics status/query/export are admin-only and use the configured ClickHouse plane for high-volume audit/feed history.
- `POST /v1/lifecycle/cache/put`, `POST /v1/lifecycle/cache/get`, `GET|POST /v1/lifecycle/cache/stats`, `POST /v1/lifecycle/cache/clear`, `POST /v1/lifecycle/semantic-cache/put`, `POST /v1/lifecycle/semantic-cache/search`, `GET|POST /v1/lifecycle/semantic-cache/stats`, and `POST /v1/lifecycle/semantic-cache/clear` expose response and semantic cache operations backed by SQLite/Postgres tables. Stats and clear are actor-scoped, so one agent can inspect or purge its own cache without touching another agent's entries. Cache entries track `accessed_at_ms`, `hit_count`, `token_count`, `tokens_saved`, semantic embedding counts, and optional actor-local `max_entries` LRU eviction.
- `GET|POST /v1/lifecycle/embedding-cache/stats` and `POST /v1/lifecycle/embedding-cache/clear` expose the shared embedding cache used by vector embed, retrieval query embeddings, auto-vector ingestion, worker extraction indexing, and vector outbox replay. Entries are keyed by a provider/model/dimensions/text hash, store only embedding JSON plus provider metadata, track hits/access time, and prune by `embedding_cache_max_entries` / `embedding_max_entries` with a default 10k LRU cap.
