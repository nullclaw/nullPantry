...
pub const descriptors = [_]EngineDescriptor{
    ...
    .{ .kind = .postgres, .role = "durable relational memory target", .durability = "durable", .nullpantry_strategy = "native libpq runtime adapter with pgvector schema; psql is an explicit dev fallback only", .remote_primary_supported = true },
