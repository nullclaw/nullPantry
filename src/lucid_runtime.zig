...
pub const max_token_budget: usize = 16 * 1024;
pub const max_local_hit_threshold: usize = 100;
pub const max_timeout_ms: u32 = 60_000;
pub const max_failure_cooldown_ms: u64 = 60 * 60 * 1000;

pub fn boundedTokenBudget(value: ?i64, fallback: usize) usize {
    if (value) |raw| {
        return @max(@as(usize, 1), bounded_int.positiveI64ToUsizeBounded(raw, max_token_budget));
    }
    return @max(@as(usize, 1), @min(fallback, max_token_budget));
}

pub fn boundedLocalHitThreshold(value: ?i64, fallback: usize) usize {
    if (value) |raw| {
        return @max(@as(usize, 1), bounded_int.positiveI64ToUsizeBounded(raw, max_local_hit_threshold));
    }
    return @max(@as(usize, 1), @min(fallback, max_local_hit_threshold));
}

pub fn validTokenBudget(value: usize) bool {
    return value > 0 and value <= max_token_budget;
}

pub fn validLocalHitThreshold(value: usize) bool {
    return value > 0 and value <= max_local_hit_threshold;
}

pub fn validTimeoutMs(value: u32) bool {
    return value > 0 and value <= max_timeout_ms;
}

pub fn validFailureCooldownMs(value: u64) bool {
    return value <= max_failure_cooldown_ms;
}
...
pub const Config = struct {
    ...
    pub fn isEnabled(self: Config) bool {
        return self.enabled and nonEmptyString(self.command);
    }

    pub fn validateUsable(self: Config) !void {
        if (!self.enabled) return;
        if (!nonEmptyString(self.command)) return error.InvalidLucidCommand;
        if (!nonEmptyString(self.workspace_dir)) return error.InvalidLucidWorkspace;
        if (!validTokenBudget(self.token_budget)) return error.InvalidLucidTokenBudget;
        if (!validLocalHitThreshold(self.local_hit_threshold)) return error.InvalidLucidLocalHitThreshold;
        if (!validTimeoutMs(self.recall_timeout_ms) or !validTimeoutMs(self.store_timeout_ms)) return error.InvalidLucidTimeout;
        if (!validFailureCooldownMs(self.failure_cooldown_ms)) return error.InvalidLucidCooldown;
    }
};
...
pub const Runtime = struct {
    ...
    fn markFailure(self: *Runtime) void {
        self.cooldown_until_ms = failureCooldownDeadlineMs(nowMs(), self.config.failure_cooldown_ms);
    }
...
fn failureCooldownDeadlineMs(now_ms: i64, cooldown_ms: u64) i64 {
    const max_cooldown_ms: u64 = @intCast(std.math.maxInt(i64));
    const bounded_cooldown_ms: i64 = @intCast(@min(cooldown_ms, max_cooldown_ms));
    return time_math.deadlineMs(now_ms, bounded_cooldown_ms);
}
...
test "lucid failure cooldown deadline is saturation safe" {
    try std.testing.expectEqual(@as(i64, 100), failureCooldownDeadlineMs(100, 0));
    try std.testing.expectEqual(@as(i64, 150), failureCooldownDeadlineMs(100, 50));
    try std.testing.expectEqual(std.math.maxInt(i64), failureCooldownDeadlineMs(std.math.maxInt(i64) - 10, 100));
    try std.testing.expectEqual(std.math.maxInt(i64), failureCooldownDeadlineMs(0, std.math.maxInt(u64)));
}

test "lucid config numeric limits are bounded" {
    try std.testing.expectEqual(@as(usize, 1), boundedTokenBudget(-1, 200));
    try std.testing.expectEqual(@as(usize, 1), boundedTokenBudget(0, 200));
    try std.testing.expectEqual(@as(usize, 512), boundedTokenBudget(512, 200));
    try std.testing.expectEqual(max_token_budget, boundedTokenBudget(std.math.maxInt(i64), 200));
    try std.testing.expectEqual(max_token_budget, boundedTokenBudget(null, max_token_budget + 1));

    try std.testing.expectEqual(@as(usize, 1), boundedLocalHitThreshold(-1, 3));
    try std.testing.expectEqual(@as(usize, 1), boundedLocalHitThreshold(0, 3));
    try std.testing.expectEqual(@as(usize, 5), boundedLocalHitThreshold(5, 3));
    try std.testing.expectEqual(max_local_hit_threshold, boundedLocalHitThreshold(std.math.maxInt(i64), 3));
    try std.testing.expectEqual(max_local_hit_threshold, boundedLocalHitThreshold(null, max_local_hit_threshold + 1));
}

test "lucid config validates usable setup" {
    try (Config{}).validateUsable();
    try (Config{ .enabled = false, .command = " " }).validateUsable();

    try std.testing.expect(!(Config{ .enabled = true, .command = " " }).isEnabled());
    try std.testing.expectError(error.InvalidLucidCommand, (Config{ .enabled = true, .command = " " }).validateUsable());
    try std.testing.expectError(error.InvalidLucidWorkspace, (Config{ .enabled = true, .workspace_dir = " " }).validateUsable());
    try std.testing.expectError(error.InvalidLucidTokenBudget, (Config{ .enabled = true, .token_budget = 0 }).validateUsable());
    try std.testing.expectError(error.InvalidLucidTokenBudget, (Config{ .enabled = true, .token_budget = max_token_budget + 1 }).validateUsable());
    try std.testing.expectError(error.InvalidLucidLocalHitThreshold, (Config{ .enabled = true, .local_hit_threshold = 0 }).validateUsable());
    try std.testing.expectError(error.InvalidLucidLocalHitThreshold, (Config{ .enabled = true, .local_hit_threshold = max_local_hit_threshold + 1 }).validateUsable());
    try std.testing.expectError(error.InvalidLucidTimeout, (Config{ .enabled = true, .recall_timeout_ms = 0 }).validateUsable());
    try std.testing.expectError(error.InvalidLucidTimeout, (Config{ .enabled = true, .store_timeout_ms = 0 }).validateUsable());
    try std.testing.expectError(error.InvalidLucidTimeout, (Config{ .enabled = true, .recall_timeout_ms = max_timeout_ms + 1 }).validateUsable());
    try (Config{ .enabled = true, .failure_cooldown_ms = 0 }).validateUsable();
    try std.testing.expectError(error.InvalidLucidCooldown, (Config{ .enabled = true, .failure_cooldown_ms = max_failure_cooldown_ms + 1 }).validateUsable());

    try (Config{ .enabled = true, .command = "lucid", .workspace_dir = "." }).validateUsable();
    _ = try Runtime.init(.{ .enabled = true, .command = "lucid", .workspace_dir = "." });
}

test "lucid projection gates writes by configured projection scopes" {
    var runtime = try Runtime.init(.{ .enabled = true, .project_scopes_json = "[\"public\"]" });
    try std.testing.expect(runtime.canProject("public", "[]"));
    try std.testing.expect(!runtime.canProject("project:secret", "[]"));
    var admin_runtime = try Runtime.init(.{ .enabled = true, .project_scopes_json = "[\"admin\"]" });
