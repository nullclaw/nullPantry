...
pub fn handleRequest(ctx: *Context, method: []const u8, target: []const u8, body: []const u8, raw_request: []const u8) HttpResponse {
    ...
    } else if (eql(seg1, "lifecycle") and eql(seg2, "snapshot") and eql(seg3, "hydrate") and is_post) {
        return lifecycleSnapshotImport(ctx, body);
    } else if (eql(seg1, "lifecycle") and eql(seg2, "hydrate") and is_post) {
        return lifecycleSnapshotImport(ctx, body);
...
fn openApiDocument(ctx: *Context) HttpResponse {
    const paths = [_]OpenApiPath{
        ...
        .{ .path = "/lifecycle/hydrate", .post = "hydrateSnapshot" },
...
fn retrievalPlan(ctx: *Context, body: []const u8) HttpResponse {
    ...
    const has_vector_index = (ctx.store.countVectorChunks() catch 0) > 0 and (json.boolField(obj, "use_vector") orelse true);
    ...
    var plan = retrieval.buildPlanWithAdaptive(ctx.allocator, query, has_vector_index, allow_reranker, .{ .enabled = adaptive_retrieval }) catch return serverError(ctx);
...
fn feedRuntimeForRoute(ctx: *Context, route: store_mod.AgentMemoryStorageRoute) !?*agent_memory_runtime.Runtime {
    return switch (route.target) {
        .primary, .native => null,
        .runtime => if (ctx.store.agent_memory.isExternal() and ctx.store.agent_memory.supportsFeed()) &ctx.store.agent_memory else null,
        .named => blk: {
            ...
            break :blk if (runtime.supportsFeed()) runtime else null;
        },
        .subset, .all => null,
    };
}
...
fn feedEventMatchesStorageRoute(ctx: *Context, event: store_mod.FeedEvent, route: store_mod.AgentMemoryStorageRoute) bool {
    return switch (route.target) {
        .primary, .all => true,
        .native => feedEventMatchesStoreName(ctx, event, "native", true),
        .runtime => feedEventMatchesStoreName(ctx, event, "runtime", false),
        .named => blk: {
            const name = route.name orelse break :blk false;
            break :blk feedEventMatchesStoreName(ctx, event, name, false);
        },
        .subset => blk: {
            for (route.stores) |store_name| {
                if (feedEventMatchesStoreName(ctx, event, store_name, std.ascii.eqlIgnoreCase(store_name, "native"))) break :blk true;
            }
            break :blk false;
        },
    };
...
fn feedEventMatchesStoreName(ctx: *Context, event: store_mod.FeedEvent, store_name: []const u8, include_unrouted_native: bool) bool {
    var parsed = std.json.parseFromSlice(std.json.Value, ctx.allocator, event.payload_json, .{}) catch return include_unrouted_native;
    defer parsed.deinit();
    if (parsed.value != .object) return include_unrouted_native;
    const obj = parsed.value.object;
    var routed = false;
    if (json.stringField(obj, "store")) |value| {
        routed = true;
        if (std.ascii.eqlIgnoreCase(value, store_name)) return true;
    }
    if (json.stringField(obj, "storage")) |value| {
        routed = true;
        if (std.ascii.eqlIgnoreCase(value, "all") or std.ascii.eqlIgnoreCase(value, store_name)) return true;
    }
    if (obj.get("stores")) |stores| {
        routed = true;
        if (stores == .array) {
            for (stores.array.items) |item| {
                if (item == .string and std.ascii.eqlIgnoreCase(item.string, store_name)) return true;
            }
        }
    }
    return include_unrouted_native and !routed;
}
...
fn buildAppliedAgentSessionUsageInput(ctx: *Context, operation: []const u8, payload_json: []const u8, event_actor_id: []const u8, fallback_route: store_mod.AgentMemoryStorageRoute) !AgentSessionUsageApplyInput {
    const payload = try std.json.parseFromSlice(std.json.Value, ctx.allocator, payload_json, .{});
    // Returned input fields borrow slices from this request-arena parse tree.
    if (payload.value != .object) return error.InvalidPayload;
    const obj = payload.value.object;
...
fn lifecycleMigrateAgentMemory(
    ...
    const fetch_limit = @min(limit + offset, 500);
    if (query.len > 0) {
        const primary = ctx.store.agentMemorySearchRouted(ctx.allocator, query, fetch_limit, session_id, scopes_json, ctx.actor_id, source_route) catch |err| switch (err) {
            error.AgentMemoryStorageUnavailable => return agentMemoryStorageUnavailable(ctx),
            else => return serverError(ctx),
        };
        appendAgentMemoryEntries(ctx.allocator, &candidates, primary) catch return serverError(ctx);
        if (include_global and session_id != null) {
            const global = ctx.store.agentMemorySearchRouted(ctx.allocator, query, fetch_limit, null, scopes_json, ctx.actor_id, source_route) catch |err| switch (err) {
                error.AgentMemoryStorageUnavailable => return agentMemoryStorageUnavailable(ctx),
                else => return serverError(ctx),
            };
        ...
        const primary = ctx.store.agentMemoryListVisibleRouted(ctx.allocator, category, session_id, ctx.actor_id, scopes_json, source_route) catch |err| switch (err) {
            error.AgentMemoryStorageUnavailable => return agentMemoryStorageUnavailable(ctx),
            else => return serverError(ctx),
        };
        appendAgentMemoryEntries(ctx.allocator, &candidates, primary) catch return serverError(ctx);
        if (include_global and session_id != null) {
            const global = ctx.store.agentMemoryListVisibleRouted(ctx.allocator, category, null, ctx.actor_id, scopes_json, source_route) catch |err| switch (err) {
                error.AgentMemoryStorageUnavailable => return agentMemoryStorageUnavailable(ctx),
                else => return serverError(ctx),
            };
            appendMissingAgentMemoryEntries(ctx.allocator, &candidates, global) catch return serverError(ctx);
    ...
    if (!dry_run) {
        for (selected.items) |entry| {
            ...
            if (delete_source) {
                _ = ctx.store.agentMemoryDeleteRouted(entry.key, entry.session_id, entry.actor_id, ctx.actor_id, source_route) catch |err| switch (err) {
                    error.AgentMemoryStorageUnavailable => return agentMemoryStorageUnavailable(ctx),
                    else => return serverError(ctx),
                };
            }
...
fn appendRuntimeDiagnostics(ctx: *Context, out: *std.ArrayListUnmanaged(u8), schema_version: i64, schema_ok: bool, store_diag: store_mod.LifecycleDiagnostics) !void {
    ...
    try out.appendSlice(ctx.allocator, "},\"retrieval\":{\"keyword\":true,\"vector\":true,\"adaptive_retrieval\":true,\"graph\":true,\"query_expansion\":true,\"rrf\":true,\"min_relevance\":true,\"temporal_decay\":true,\"mmr\":true,\"llm_rerank_configured\":");
...
fn lifecycleSnapshotImport(ctx: *Context, body: []const u8) HttpResponse {
    ...
    defer parsed.deinit();
    const obj = parsed.value.object;
    const objects_value = snapshotObjectsValue(obj) orelse return json.errorResponse(ctx.allocator, 400, "bad_request", "Missing objects");
    ...
    const dry_run = (json.boolField(obj, "dry_run") orelse false) or
    ...
    for (objects.items) |item| {
        ...
        const exported_source_ids_json = rawField(ctx.allocator, source_obj, "source_ids", "[]") catch return serverError(ctx);
        var source_ids_json = rawField(ctx.allocator, source_obj, "citations", exported_source_ids_json) catch return serverError(ctx);
...
fn snapshotHydrateContextPack(ctx: *Context, obj: std.json.ObjectMap, options: SnapshotImportOptions) !SnapshotHydratedObject {
        ...
        obj.get("included_result_refs_json") != null)
    {
        ...
        const pack = try ctx.store.importContextPackSnapshot(ctx.allocator, .{
            ...
            .included_sources_json = try rawFieldAny(ctx.allocator, obj, &.{ "included_sources", "included_sources_json", "citations" }, "[]"),
            .included_artifacts_json = try rawFieldAny(ctx.allocator, obj, &.{ "included_artifacts", "included_artifacts_json" }, "[]"),
            .included_memory_atoms_json = try rawFieldAny(ctx.allocator, obj, &.{ "included_memory_atoms", "included_memory_atoms_json" }, "[]"),
            .included_result_refs_json = try rawFieldAny(ctx.allocator, obj, &.{ "included_result_refs", "included_result_refs_json" }, "[]"),
...
fn lifecycleRollout(ctx: *Context, body: []const u8) HttpResponse {
    ...
    out.appendSlice(ctx.allocator, ",\"policy\":{\"mode\":") catch return serverError(ctx);
    json.appendString(&out, ctx.allocator, policy.mode.name()) catch return serverError(ctx);
    out.print(ctx.allocator, ",\"percent\":{d},\"shadow_percent\":{d},\"disabled\":{s},\"salt\":", .{ policy.percent, policy.shadow_percent, if (policy.disabled) "true" else "false" }) catch return serverError(ctx);
    json.appendString(&out, ctx.allocator, policy.salt) catch return serverError(ctx);
    out.appendSlice(ctx.allocator, ",\"required_scopes\":") catch return serverError(ctx);
    json.appendRawJsonOr(&out, ctx.allocator, policy.required_scopes_json, "[]") catch return serverError(ctx);
    out.appendSlice(ctx.allocator, ",\"target_scopes\":") catch return serverError(ctx);
    json.appendRawJsonOr(&out, ctx.allocator, policy.target_scopes_json, "[]") catch return serverError(ctx);
    out.appendSlice(ctx.allocator, ",\"blocked_scopes\":") catch return serverError(ctx);
    json.appendRawJsonOr(&out, ctx.allocator, policy.blocked_scopes_json, "[]") catch return serverError(ctx);
    out.appendSlice(ctx.allocator, ",\"required_capabilities\":") catch return serverError(ctx);
    json.appendRawJsonOr(&out, ctx.allocator, policy.required_capabilities_json, "[]") catch return serverError(ctx);
    out.appendSlice(ctx.allocator, ",\"blocked_capabilities\":") catch return serverError(ctx);
    json.appendRawJsonOr(&out, ctx.allocator, policy.blocked_capabilities_json, "[]") catch return serverError(ctx);
...
fn agentMemoryEntryMigratable(ctx: *Context, entry: domain.AgentMemory) bool {
    const actor_owned = std.mem.eql(u8, entry.actor_id, ctx.actor_id) and
        domain.isActorOwnedAgentMemoryScope(entry.scope, ctx.actor_id) and
        access.permissionsVisibleForActor(ctx.allocator, entry.permissions_json, ctx.actor_scopes_json, ctx.actor_id);
    if (actor_owned) return true;
    ...
    return canWriteRecord(ctx, entry.scope, entry.permissions_json);
}
...
fn appendSnapshotFullRecord(ctx: *Context, out: *std.ArrayListUnmanaged(u8), result: domain.SearchResult) !bool {
    ...
    if (std.mem.eql(u8, result.result_type, "agent_memory")) {
        const entry = (try ctx.store.agentMemoryById(ctx.allocator, result.id)) orelse return false;
...
fn appendContextPackSnapshotRecord(ctx: *Context, out: *std.ArrayListUnmanaged(u8), pack: store_mod.ContextPackResult) !void {
    ...
    try out.appendSlice(ctx.allocator, ",\"permissions\":");
    try json.appendRawJsonOr(out, ctx.allocator, permissions, "[]");
    try out.appendSlice(ctx.allocator, ",\"scopes\":");
    try json.appendRawJsonOr(out, ctx.allocator, pack.required_scopes_json, "[]");
    try out.appendSlice(ctx.allocator, ",\"required_scopes\":");
    try json.appendRawJsonOr(out, ctx.allocator, pack.required_scopes_json, "[]");
    try out.appendSlice(ctx.allocator, ",\"included_sources\":");
    try json.appendRawJsonOr(out, ctx.allocator, pack.included_sources_json, "[]");
    try out.appendSlice(ctx.allocator, ",\"included_artifacts\":");
    try json.appendRawJsonOr(out, ctx.allocator, pack.included_artifacts_json, "[]");
    try out.appendSlice(ctx.allocator, ",\"included_memory_atoms\":");
    try json.appendRawJsonOr(out, ctx.allocator, pack.included_memory_atoms_json, "[]");
    try out.appendSlice(ctx.allocator, ",\"included_result_refs\":");
    try json.appendRawJsonOr(out, ctx.allocator, pack.included_result_refs_json, "[]");
    try out.appendSlice(ctx.allocator, ",\"actor_id\":");
...
fn contextPackSnapshotScope(allocator: std.mem.Allocator, pack: store_mod.ContextPackResult) ![]const u8 {
    ...
    return firstJsonStringOrDefault(allocator, pack.required_scopes_json, "public");
}
...
fn firstJsonStringOrDefault(allocator: std.mem.Allocator, value: []const u8, fallback: []const u8) ![]const u8 {
    const parsed = std.json.parseFromSlice(std.json.Value, allocator, value, .{}) catch return allocator.dupe(u8, fallback);
    defer parsed.deinit();
...
fn buildAppliedContextPackInput(ctx: *Context, obj: std.json.ObjectMap, event_actor_id: []const u8, event_object_id: ?[]const u8, fallback_route: store_mod.AgentMemoryStorageRoute) !store_mod.ContextPackInput {
    ...
    return .{
        ...
        .adaptive_retrieval = adaptiveRetrievalFromObject(obj),
...
fn rawFieldAny(allocator: std.mem.Allocator, obj: std.json.ObjectMap, names: []const []const u8, fallback: []const u8) ![]const u8 {
    for (names) |name| {
...
fn buildSearchInput(ctx: *Context, obj: std.json.ObjectMap, query: []const u8, limit: usize, include_sessions_default: bool) !store_mod.SearchInput {
    ...
    const analysis = retrieval.analyzeQuery(query, .{ .enabled = adaptive_retrieval });
...
test "api lifecycle migrate moves agent memory between storage planes" {
    var store = try Store.initSQLiteWithOptions(std.testing.allocator, ":memory:", .{
        .agent_memory = .{ .backend = .memory_lru },
        .agent_memory_stores = &.{
            .{ .name = "scratch", .config = .{ .backend = .memory_lru } },
    ...
    ;
    var ctx = Context{ .allocator = alloc, .store = &store, .token_principals_json = principals };
    const raw = "PUT /v1/agent-memory/migrate.key HTTP/1.1\r\nAuthorization: Bearer agent-route\r\n\r\n{}";

    const put = handleRequest(&ctx, "PUT", "/v1/agent-memory/migrate.key", "{\"content\":\"Migrate Scratch Unique\",\"scope\":\"public\",\"store\":\"scratch\"}", raw);
    try std.testing.expectEqualStrings("200 OK", put.status);

    const dry_run = handleRequest(&ctx, "POST", "/v1/lifecycle/migrate", "{\"from\":\"scratch\",\"to\":\"archive\",\"query\":\"Migrate Scratch Unique\",\"scopes\":[\"public\"],\"dry_run\":true,\"limit\":10}", "POST /v1/lifecycle/migrate HTTP/1.1\r\nAuthorization: Bearer agent-route\r\n\r\n{}");
    ...
    try std.testing.expect(std.mem.indexOf(u8, dry_run.body, "\"source_store\":\"scratch\"") != null);

    const archive_before = handleRequest(&ctx, "POST", "/v1/agent-memory/search", "{\"query\":\"Migrate Scratch Unique\",\"store\":\"archive\",\"limit\":10}", "POST /v1/agent-memory/search HTTP/1.1\r\nAuthorization: Bearer agent-route\r\n\r\n{}");
    try std.testing.expectEqualStrings("200 OK", archive_before.status);
    try std.testing.expect(std.mem.indexOf(u8, archive_before.body, "Migrate Scratch Unique") == null);

    const execute = handleRequest(&ctx, "POST", "/v1/lifecycle/migrate", "{\"from_store\":\"scratch\",\"to_store\":\"archive\",\"query\":\"Migrate Scratch Unique\",\"scopes\":[\"public\"],\"execute\":true,\"delete_source\":true}", "POST /v1/lifecycle/migrate HTTP/1.1\r\nAuthorization: Bearer agent-route\r\n\r\n{}");
    ...
    try std.testing.expect(std.mem.indexOf(u8, execute.body, "\"target_store\":\"archive\"") != null);

    const archive_after = handleRequest(&ctx, "POST", "/v1/agent-memory/search", "{\"query\":\"Migrate Scratch Unique\",\"store\":\"archive\",\"limit\":10}", "POST /v1/agent-memory/search HTTP/1.1\r\nAuthorization: Bearer agent-route\r\n\r\n{}");
    try std.testing.expectEqualStrings("200 OK", archive_after.status);
    try std.testing.expect(std.mem.indexOf(u8, archive_after.body, "Migrate Scratch Unique") != null);
    const scratch_after = handleRequest(&ctx, "GET", "/v1/agent-memory/migrate.key?store=scratch&scope=public", "", "GET /v1/agent-memory/migrate.key?store=scratch&scope=public HTTP/1.1\r\nAuthorization: Bearer agent-route\r\n\r\n");
...
test "api memory feed endpoints honor named runtime route selection" {
    ...
    var ctx = Context{
        .allocator = alloc,
        .store = &store,
        .actor_id = "agent:route",
        .actor_scopes_json = "[\"public\",\"write:public\",\"session:*\",\"write:session:*\"]",
...
test "api exposes engine registry retrieval plan vector and lifecycle endpoints" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, openapi_resp.body, "\"adaptive_retrieval\"") != null);
...
test "api lifecycle snapshot hydrate preserves typed primitives" {
    ...
    const body =
        ...
        \\  {"object_type":"context_pack","record":{"id":"ctx_snapshot_typed","purpose":"task","target":"agent","query":"typed snapshot context query","generated_summary":"Typed context pack exact summary survives hydrate","scope":"public","permissions":[],"required_scopes":["public"],"included_sources":["src_snapshot_typed"],"included_artifacts":["art_snapshot_typed"],"included_memory_atoms":["mem_snapshot_typed"],"included_result_refs":[]}}
        \\]}
    ;
