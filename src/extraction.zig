...
pub fn deinitOwnedParsedMemory(allocator: std.mem.Allocator, memory: *ParsedMemory) void {
    allocator.free(memory.predicate);
    allocator.free(memory.object);
    allocator.free(memory.text);
    allocator.free(memory.tags_json);
    if (memory.evidence) |value| allocator.free(value);
    memory.* = undefined;
}

pub fn freeOwnedParsedMemories(allocator: std.mem.Allocator, memories: []ParsedMemory) void {
    for (memories) |*memory| deinitOwnedParsedMemory(allocator, memory);
...
pub fn deinitOwnedParsedRelation(allocator: std.mem.Allocator, relation: *ParsedRelation) void {
    allocator.free(relation.from_name);
    allocator.free(relation.relation_type);
    allocator.free(relation.to_name);
    allocator.free(relation.text);
    if (relation.evidence) |value| allocator.free(value);
    relation.* = undefined;
}

pub fn freeOwnedParsedRelations(allocator: std.mem.Allocator, relations: []ParsedRelation) void {
    for (relations) |*relation| deinitOwnedParsedRelation(allocator, relation);
    allocator.free(relations);
}

pub const GroundedStructuredExtraction = struct {
    memories: []ParsedMemory,
    relations: []ParsedRelation,
    skipped_ungrounded: usize,

    pub fn deinit(self: *GroundedStructuredExtraction, allocator: std.mem.Allocator) void {
        freeOwnedParsedMemories(allocator, self.memories);
        freeOwnedParsedRelations(allocator, self.relations);
        self.* = .{ .memories = &.{}, .relations = &.{}, .skipped_ungrounded = 0 };
    }

    pub fn acceptedCount(self: GroundedStructuredExtraction) usize {
        return self.memories.len + self.relations.len;
    }
};
...
pub const ResultCounts = struct {
    ...
    pub fn addVectorChunks(self: *ResultCounts, delta: usize) void {
        self.vector_chunk_count = countAdd(self.vector_chunk_count, delta);
    }
};

fn countAdd(total: usize, delta: usize) usize {
    return bounded_int.saturatingUsizeAdd(total, delta);
}
...
pub fn resultJson(
    ...
    try out.print(
        allocator,
        ",\"artifact_count\":{d},\"memory_atom_count\":{d},\"entity_count\":{d},\"relation_count\":{d},\"vector_chunk_count\":{d},\"extraction_provider\":",
        .{ counts.artifact_count, counts.memory_atom_count, counts.entity_count, counts.relation_count, counts.vector_chunk_count },
    );
    try json.appendString(&out, allocator, extraction_provider);
    try out.appendSlice(allocator, ",\"extraction_fallback\":");
    try out.appendSlice(allocator, if (extraction_fallback) "true}" else "false}");
    return out.toOwnedSlice(allocator);
}
...
pub const SourceLineRange = struct {
    text: []const u8,
    start: usize,
    end: usize,
    line_no: usize,
...
pub const SourceLineIterator = struct {
    ...
    pub fn init(content: []const u8) SourceLineIterator {
        return .{
            .content = content,
    ...
    pub fn next(self: *SourceLineIterator) ?SourceLineRange {
        const text = self.lines.next() orelse return null;
        const start = @min(self.offset, self.content.len);
        const end = @min(checkedAddMax(start, text.len), self.content.len);
        const line_no = self.line_no;
        self.offset = @min(checkedAddMax(end, 1), self.content.len);
        self.line_no = checkedAddMax(self.line_no, 1);
        return .{
            .text = text,
            .start = start,
            .end = end,
...
pub fn sourceLineIterator(content: []const u8) SourceLineIterator {
    return SourceLineIterator.init(content);
}
...
pub fn groundedEvidenceRangeForText(allocator: std.mem.Allocator, source_id: []const u8, content: []const u8, evidence_text: ?[]const u8) ![]u8 {
    const quote = if (evidence_text) |text| std.mem.trim(u8, text, " \t\r\n") else "";
    if (quote.len == 0) return error.UngroundedEvidence;
    const start = std.mem.indexOf(u8, content, quote) orelse return error.UngroundedEvidence;
    return evidenceRangeJson(allocator, source_id, start, start + quote.len, lineNumberAt(content, start));
}

pub fn evidenceGroundedInSource(content: []const u8, evidence_text: ?[]const u8) bool {
    const quote = if (evidence_text) |text| std.mem.trim(u8, text, " \t\r\n") else "";
    return quote.len > 0 and std.mem.indexOf(u8, content, quote) != null;
}
...
fn checkedAddMax(a: usize, b: usize) usize {
    return bounded_int.saturatingUsizeAdd(a, b);
}
...
pub fn memoryExtractionPrompt(allocator: std.mem.Allocator, title: []const u8, source_type: []const u8, content: []const u8) ![]const u8 {
    ...
    const safe_title = try redactor.redact(allocator, title);
    defer allocator.free(safe_title);
    const safe_source_type = try redactor.redact(allocator, source_type);
    defer allocator.free(safe_source_type);
    const safe_content = try redactor.redact(allocator, content);
    ...
    return std.fmt.allocPrint(allocator,
        ...
        \\Source:
        \\{s}
    , .{ safe_title, safe_source_type, safe_content });
}
...
pub fn parseStructuredMemoryResponse(allocator: std.mem.Allocator, body: []const u8) ![]ParsedMemory {
    ...
    errdefer {
        for (out.items) |*memory| deinitOwnedParsedMemory(allocator, memory);
    ...
    for (array.items) |item| {
        ...
        const predicate_owned = try allocator.dupe(u8, if (predicate.len == 0) "states" else predicate);
        errdefer allocator.free(predicate_owned);
        const object_owned = try allocator.dupe(u8, if (object.len == 0) normalized_text else object);
        errdefer allocator.free(object_owned);
        const text_owned = try allocator.dupe(u8, normalized_text);
        errdefer allocator.free(text_owned);
        const evidence_owned = if (json.stringField(obj, "evidence") orelse json.stringField(obj, "source_quote")) |evidence| try allocator.dupe(u8, evidence) else null;
        errdefer if (evidence_owned) |value| allocator.free(value);
        try out.append(allocator, .{
            .predicate = predicate_owned,
            .object = object_owned,
            .text = text_owned,
            .confidence = confidence,
            .tags_json = tags_json,
            .evidence = evidence_owned,
        });
    }
    return out.toOwnedSlice(allocator);
}
...
pub fn parseStructuredRelationsResponse(allocator: std.mem.Allocator, body: []const u8) ![]ParsedRelation {
    ...
    errdefer {
        for (out.items) |*relation| deinitOwnedParsedRelation(allocator, relation);
        out.deinit(allocator);
    }
    for (array.items) |item| {
        if (item != .object) continue;
        ...
        const generated_text = if (evidence == null) try std.fmt.allocPrint(allocator, "{s} {s} {s}", .{ from_name, relation_type, to_name }) else null;
        defer if (generated_text) |value| allocator.free(value);
        const text = evidence orelse generated_text.?;
        const from_name_owned = try allocator.dupe(u8, from_name);
        errdefer allocator.free(from_name_owned);
        const relation_type_owned = try allocator.dupe(u8, relation_type);
        errdefer allocator.free(relation_type_owned);
        const to_name_owned = try allocator.dupe(u8, to_name);
        errdefer allocator.free(to_name_owned);
        const text_owned = try allocator.dupe(u8, text);
        errdefer allocator.free(text_owned);
        const evidence_owned = if (evidence) |value| try allocator.dupe(u8, value) else null;
        errdefer if (evidence_owned) |value| allocator.free(value);
        try out.append(allocator, .{
            .from_name = from_name_owned,
            .relation_type = relation_type_owned,
            .to_name = to_name_owned,
            .text = text_owned,
...
pub fn parseGroundedStructuredResponse(allocator: std.mem.Allocator, source_content: []const u8, body: []const u8, strict: bool) !GroundedStructuredExtraction {
    const parsed_memories = try parseStructuredMemoryResponse(allocator, body);
    defer freeOwnedParsedMemories(allocator, parsed_memories);
    const parsed_relations = parseStructuredRelationsResponse(allocator, body) catch try allocator.alloc(ParsedRelation, 0);
    defer freeOwnedParsedRelations(allocator, parsed_relations);

    var memories: std.ArrayListUnmanaged(ParsedMemory) = .empty;
    var relations: std.ArrayListUnmanaged(ParsedRelation) = .empty;
    var skipped_ungrounded: usize = 0;
    errdefer {
        for (memories.items) |*memory| deinitOwnedParsedMemory(allocator, memory);
        memories.deinit(allocator);
        for (relations.items) |*relation| deinitOwnedParsedRelation(allocator, relation);
        relations.deinit(allocator);
    }

    for (parsed_memories) |memory| {
        if (!evidenceGroundedInSource(source_content, memory.evidence orelse memory.text)) {
            skipped_ungrounded += 1;
            if (strict) return error.UngroundedEvidence;
            continue;
        }
        try memories.append(allocator, try cloneParsedMemory(allocator, memory));
    }
    for (parsed_relations) |relation| {
        if (!evidenceGroundedInSource(source_content, relation.evidence orelse relation.text)) {
            skipped_ungrounded += 1;
            if (strict) return error.UngroundedEvidence;
            continue;
        }
        try relations.append(allocator, try cloneParsedRelation(allocator, relation));
    }

    return .{
        .memories = try memories.toOwnedSlice(allocator),
        .relations = try relations.toOwnedSlice(allocator),
        .skipped_ungrounded = skipped_ungrounded,
    };
}

fn cloneParsedMemory(allocator: std.mem.Allocator, memory: ParsedMemory) !ParsedMemory {
    const predicate = try allocator.dupe(u8, memory.predicate);
    errdefer allocator.free(predicate);
    const object = try allocator.dupe(u8, memory.object);
    errdefer allocator.free(object);
    const text = try allocator.dupe(u8, memory.text);
    errdefer allocator.free(text);
    const tags_json = try allocator.dupe(u8, memory.tags_json);
    errdefer allocator.free(tags_json);
    const evidence = if (memory.evidence) |value| try allocator.dupe(u8, value) else null;
...
fn cloneParsedRelation(allocator: std.mem.Allocator, relation: ParsedRelation) !ParsedRelation {
    const from_name = try allocator.dupe(u8, relation.from_name);
    errdefer allocator.free(from_name);
    const relation_type = try allocator.dupe(u8, relation.relation_type);
    errdefer allocator.free(relation_type);
    const to_name = try allocator.dupe(u8, relation.to_name);
    errdefer allocator.free(to_name);
    const text = try allocator.dupe(u8, relation.text);
    errdefer allocator.free(text);
    const evidence = if (relation.evidence) |value| try allocator.dupe(u8, value) else null;
    errdefer if (evidence) |value| allocator.free(value);
    return .{
        .from_name = from_name,
...
pub fn extractEntityNamesJson(allocator: std.mem.Allocator, content: []const u8) ![]u8 {
    ...
    while (tokens.next()) |raw| {
        ...
        if (try containsJsonString(allocator, out.items, token)) continue;
...
fn containsJsonString(allocator: std.mem.Allocator, haystack: []const u8, value: []const u8) !bool {
...
test "extraction result json escapes string fields" {
    const result = try resultJson(
        std.testing.allocator,
        "src_\"quoted\"",
        .{ .artifact_count = 1, .memory_atom_count = 2, .entity_count = 3, .relation_count = 4, .vector_chunk_count = 5 },
        "llm:\"provider\"",
        true,
    );
    ...
    defer parsed.deinit();
    try std.testing.expectEqualStrings("src_\"quoted\"", parsed.value.object.get("source_id").?.string);
    try std.testing.expectEqualStrings("llm:\"provider\"", parsed.value.object.get("extraction_provider").?.string);
    try std.testing.expectEqual(@as(i64, 5), parsed.value.object.get("vector_chunk_count").?.integer);
    try std.testing.expect(parsed.value.object.get("extraction_fallback").?.bool);
}

test "extraction result counts saturate vector chunks" {
    var counts = ResultCounts{};
    counts.addVectorChunks(3);
    try std.testing.expectEqual(@as(usize, 3), counts.vector_chunk_count);
...
test "extraction LLM prompt redacts model-boundary PII and secrets" {
    const prompt = try memoryExtractionPrompt(std.testing.allocator, "Owner alice@example.com", "transcript", "Decision: contact alice@example.com with token=abc123 and sk-live-secret");
...
test "extraction source line iterator reports evidence offsets" {
    const content = "Intro\nDecision: keep\nTail";
    var lines = sourceLineIterator(content);

    const first = lines.next().?;
    try std.testing.expectEqualStrings("Intro", first.text);
    try std.testing.expectEqual(@as(usize, 0), first.start);
    try std.testing.expectEqual(@as(usize, 5), first.end);
    try std.testing.expectEqual(@as(usize, 1), first.line_no);

    const second = lines.next().?;
    try std.testing.expectEqualStrings("Decision: keep", second.text);
    try std.testing.expectEqual(@as(usize, 6), second.start);
    try std.testing.expectEqual(@as(usize, 20), second.end);
    try std.testing.expectEqual(@as(usize, 2), second.line_no);

    const third = lines.next().?;
    try std.testing.expectEqualStrings("Tail", third.text);
    try std.testing.expectEqual(@as(usize, 21), third.start);
    try std.testing.expectEqual(@as(usize, content.len), third.end);
    try std.testing.expectEqual(@as(usize, 3), third.line_no);

    try std.testing.expect(lines.next() == null);
}

test "extraction source line iterator bounds trailing empty line" {
    const content = "Decision: keep\n";
    var lines = sourceLineIterator(content);

    const first = lines.next().?;
    try std.testing.expectEqualStrings("Decision: keep", first.text);
    try std.testing.expectEqual(@as(usize, 0), first.start);
    try std.testing.expectEqual(@as(usize, 14), first.end);
    try std.testing.expectEqual(@as(usize, 1), first.line_no);

    const second = lines.next().?;
    try std.testing.expectEqualStrings("", second.text);
    try std.testing.expectEqual(@as(usize, content.len), second.start);
    try std.testing.expectEqual(@as(usize, content.len), second.end);
    try std.testing.expectEqual(@as(usize, 2), second.line_no);

    try std.testing.expect(lines.next() == null);
}

test "extraction strict grounded evidence rejects invented LLM citations" {
    const alloc = std.testing.allocator;
    const content = "Intro\nDecision: keep NullPantry headless\nTail";
    const grounded = try groundedEvidenceRangeForText(alloc, "src_grounded", content, "keep NullPantry headless");
    defer alloc.free(grounded);
    try std.testing.expect(std.mem.indexOf(u8, grounded, "\"line\":2") != null);

    try std.testing.expect(evidenceGroundedInSource(content, "Decision: keep NullPantry headless"));
    try std.testing.expect(!evidenceGroundedInSource(content, "Decision: use invented remote memory"));
    try std.testing.expectError(error.UngroundedEvidence, groundedEvidenceRangeForText(alloc, "src_grounded", content, "Decision: use invented remote memory"));
    try std.testing.expectError(error.UngroundedEvidence, groundedEvidenceRangeForText(alloc, "src_grounded", content, null));
}

test "extraction grounded structured response filters ungrounded LLM output" {
    const alloc = std.testing.allocator;
    const content =
        \\Decision: NullPantry keeps source-backed memory
        \\NullTickets implements NP-42
    ;
    const body =
        \\{"memory_atoms":[
        \\{"predicate":"decision","object":"source-backed memory","text":"Decision: NullPantry keeps source-backed memory","evidence":"NullPantry keeps source-backed memory"},
        \\{"predicate":"decision","object":"invented","text":"Decision: NullPantry stores hidden remote facts","evidence":"NullPantry stores hidden remote facts"}
        \\],"relations":[
        \\{"from":"NullTickets","relation_type":"implements","to":"NP-42","evidence":"NullTickets implements NP-42"}
        \\]}
    ;
    var grounded = try parseGroundedStructuredResponse(alloc, content, body, false);
    defer grounded.deinit(alloc);
    try std.testing.expectEqual(@as(usize, 1), grounded.memories.len);
    try std.testing.expectEqual(@as(usize, 1), grounded.relations.len);
    try std.testing.expectEqual(@as(usize, 1), grounded.skipped_ungrounded);
    try std.testing.expectEqualStrings("source-backed memory", grounded.memories[0].object);
    try std.testing.expectEqualStrings("implements", grounded.relations[0].relation_type);

    try std.testing.expectError(error.UngroundedEvidence, parseGroundedStructuredResponse(alloc, content, body, true));
}
