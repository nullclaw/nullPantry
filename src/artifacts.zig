...
const rollout_policy_statuses = [_][]const u8{ "draft", "active", "paused", "deprecated" };
...
const rollout_policy_fields = [_][]const u8{ "name", "feature", "mode", "percent", "shadow_percent" };
...
pub const artifact_types = [_]ArtifactTypeDescriptor{
    ...
    .{ .name = "rollout_policy", .statuses = &rollout_policy_statuses, .required_fields = &rollout_policy_fields },
};
...
test "artifact type status validation covers first-class types" {
    ...
    try std.testing.expect(validStatus("rollout_policy", "active"));
    try std.testing.expect(!validStatus("rollout_policy", "accepted"));
    ...
    fields.clearRetainingCapacity();
    try appendRequiredFieldsJson(std.testing.allocator, &fields, "rollout_policy");
    try std.testing.expect(std.mem.indexOf(u8, fields.items, "shadow_percent") != null);
}
...
test "artifact types json is generated from descriptor registry" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, out.items, "\"type\":\"rollout_policy\"") != null);
}
