...
pub const supported_object_types_json = "[\"memory_atom\",\"source\",\"artifact\",\"entity\",\"relation\",\"agent_memory\",\"agent_session_message\",\"agent_session_usage\",\"context_pack\",\"space\",\"policy_scope\"]";
pub const knowledge_object_types_json = "[\"memory_atom\",\"source\",\"artifact\",\"entity\",\"relation\",\"context_pack\"]";
pub const runtime_object_types_json = "[\"agent_memory\",\"agent_session_message\",\"agent_session_usage\"]";
pub const lifecycle_object_types_json = supported_object_types_json;
pub const runtime_lifecycle_object_types_json = runtime_object_types_json;

pub fn feedSequenceAfter(sequence: i64) i64 {
    return std.math.add(i64, sequence, 1) catch std.math.maxInt(i64);
}

pub fn feedNextLocalOriginSequence(configured_next: i64, max_event_id: i64) i64 {
    if (configured_next > 0) return configured_next;
    return feedSequenceAfter(max_event_id);
}

pub fn feedNextLocalOriginSequenceFromFrontier(origin_frontier: i64, max_event_id: i64) i64 {
    const next_after_event_id = feedSequenceAfter(max_event_id);
    const next_after_frontier = if (origin_frontier > 0) feedSequenceAfter(origin_frontier) else 0;
    return @max(next_after_event_id, next_after_frontier);
}
...
pub fn agentMemoryWrite(allocator: std.mem.Allocator, store_name: []const u8, entry: domain.AgentMemory, operation: domain.AgentMemoryOperation, operation_content: []const u8) ![]const u8 {
    ...
    switch (operation) {
        ...
        .merge_string_set => {
            ...
            try json.appendRawJsonArray(&out, allocator, operation_content);
        },
        .merge_object => {
            try out.appendSlice(allocator, ",\"object\":");
            try json.appendRawJsonObject(&out, allocator, operation_content);
        },
    }
...
pub fn agentMemoryDelete(allocator: std.mem.Allocator, store_name: []const u8, key: []const u8, session_id: ?[]const u8, owner_actor_id: []const u8, scope: []const u8, permissions_json: []const u8) ![]const u8 {
    ...
    try out.appendSlice(allocator, ",\"permissions\":");
    try json.appendRawJsonArray(&out, allocator, permissions_json);
    try out.appendSlice(allocator, ",\"owner_id\":");
...
pub fn policyScope(allocator: std.mem.Allocator, policy: anytype) ![]const u8 {
    ...
    try out.appendSlice(allocator, ",\"permissions\":");
    try json.appendRawJsonArray(&out, allocator, policy.permissions_json);
    ...
    try json.appendRawJsonObject(&out, allocator, policy.metadata_json);
    try out.append(allocator, '}');
    return out.toOwnedSlice(allocator);
}

pub fn policyScopeDedupeKey(allocator: std.mem.Allocator, scope: []const u8, payload_json: []const u8) ![]const u8 {
    const payload_hash = digest.sha256PartsHex(&.{ scope, payload_json });
    return std.fmt.allocPrint(allocator, "direct:policy_scope:{s}:sha256:{s}", .{ scope, payload_hash[0..] });
}

test "policy scope dedupe key uses sha256 payload digest" {
    const allocator = std.testing.allocator;
    const key = try policyScopeDedupeKey(allocator, "team:alpha", "{\"visibility\":\"team\"}");
    defer allocator.free(key);
    const prefix = "direct:policy_scope:team:alpha:sha256:";
    try std.testing.expect(std.mem.startsWith(u8, key, prefix));
    try std.testing.expectEqual(prefix.len + 64, key.len);
}
...
pub fn withStorageRoute(allocator: std.mem.Allocator, payload_json: []const u8, route: anytype) ![]const u8 {
    ...
    errdefer out.deinit(allocator);
    try out.append(allocator, '{');
    var first = inner.len == 0;
    if (inner.len > 0) {
        try out.appendSlice(allocator, inner);
    }
    try storage_routes.appendCanonicalObjectFields(allocator, &out, route, &first);
    try out.append(allocator, '}');
    return out.toOwnedSlice(allocator);
}

pub fn withStorageRouteOverride(allocator: std.mem.Allocator, payload_json: []const u8, route: anytype) ![]const u8 {
    const trimmed = std.mem.trim(u8, payload_json, " \t\r\n");
    if (trimmed.len < 2 or trimmed[0] != '{' or trimmed[trimmed.len - 1] != '}') {
        return allocator.dupe(u8, payload_json);
    }
    var parsed = std.json.parseFromSlice(std.json.Value, allocator, trimmed, .{}) catch |err| switch (err) {
        ...
        else => return allocator.dupe(u8, payload_json),
    };
    defer parsed.deinit();
    if (parsed.value != .object) return allocator.dupe(u8, payload_json);
    ...
    while (iterator.next()) |entry| {
        if (storage_routes.isSelectorField(entry.key_ptr.*)) continue;
        if (!first) try out.append(allocator, ',');
        first = false;
        try json.appendString(&out, allocator, entry.key_ptr.*);
        try out.append(allocator, ':');
        const raw = try json.jsonFromValue(allocator, entry.value_ptr.*);
...
fn hasTopLevelStorageRoute(allocator: std.mem.Allocator, payload_json: []const u8) !bool {
    ...
    while (iterator.next()) |entry| {
        if (storage_routes.isSelectorField(entry.key_ptr.*)) return true;
    }
    return false;
}
...
pub fn operationFromEventType(event_type: []const u8) []const u8 {
    ...
    if (std.mem.endsWith(u8, event_type, ".delete") or
        std.mem.endsWith(u8, event_type, "_delete")) return "delete";
    if (std.mem.endsWith(u8, event_type, ".forget") or
        std.mem.endsWith(u8, event_type, "_forget")) return "forget";
...
pub fn isDeleteOperation(operation: []const u8) bool {
    return std.mem.eql(u8, operation, "delete") or std.mem.eql(u8, operation, "forget");
}

pub fn isDeleteAllOperation(operation: []const u8) bool {
    return std.mem.eql(u8, operation, "delete_all");
}

pub fn isDeleteScopedOperation(operation: []const u8) bool {
    return std.mem.eql(u8, operation, "delete_scoped");
}

pub fn isAgentMemoryDeleteOperation(operation: []const u8) bool {
    return isDeleteOperation(operation) or isDeleteAllOperation(operation) or isDeleteScopedOperation(operation);
}

pub fn isAgentSessionDeleteOperation(operation: []const u8) bool {
    return isDeleteOperation(operation) or isDeleteAllOperation(operation);
}

pub fn isLifecycleOperation(operation: []const u8) bool {
    return isDeleteOperation(operation) or
...
pub fn statusFromLifecycleOperation(operation: []const u8, payload_obj: std.json.ObjectMap) []const u8 {
    ...
    if (isDeleteOperation(operation)) return "deprecated";
    return "proposed";
}

pub fn agentMemoryLifecycleOperationForStatus(status: []const u8) []const u8 {
...
pub fn eventPermissions(allocator: std.mem.Allocator, obj: std.json.ObjectMap, object_type: []const u8, payload_json: []const u8, event_actor_id: []const u8) ![]const u8 {
    if (try eventPermissionsFromObject(allocator, obj)) |permissions| return permissions;
    const payload = try std.json.parseFromSlice(std.json.Value, allocator, payload_json, .{});
    defer payload.deinit();
    if (payload.value == .object) {
        if (try eventPermissionsFromObject(allocator, payload.value.object)) |permissions| return permissions;
    }
    if (isAgentSessionObject(object_type)) return domain.actorGrantJson(allocator, event_actor_id);
    return allocator.dupe(u8, "[]");
}

fn eventPermissionsFromObject(allocator: std.mem.Allocator, obj: std.json.ObjectMap) !?[]const u8 {
    if (obj.get("permissions")) |value| {
        if (value != .null) return try rawArrayJsonOrEncodedString(allocator, "permissions", value, "[]");
    }
    if (obj.get("permissions_json")) |value| {
        if (value != .null) return try rawArrayJsonOrEncodedString(allocator, "permissions_json", value, "[]");
    }
    return null;
}

fn rawJsonOrEncodedString(allocator: std.mem.Allocator, name: []const u8, value: std.json.Value, fallback: []const u8) ![]const u8 {
    return try json.rawJsonFieldValue(allocator, name, value, fallback);
}

fn rawArrayJsonOrEncodedString(allocator: std.mem.Allocator, name: []const u8, value: std.json.Value, fallback: []const u8) ![]const u8 {
    const raw = try rawJsonOrEncodedString(allocator, name, value, fallback);
    errdefer allocator.free(raw);
    if (!json.rawJsonRootIs(allocator, raw, .array)) return error.InvalidRawJson;
    return raw;
}
...
pub fn nullClawMemoryOperationSupported(operation: []const u8) bool {
    return std.mem.eql(u8, operation, "put") or
        ...
        std.mem.eql(u8, operation, "delete_all");
}

pub fn nullClawMemoryApplyOperationSupported(operation: []const u8) bool {
    return nullClawMemoryOperationSupported(operation) or
...
pub fn contextPack(allocator: std.mem.Allocator, pack: anytype) ![]const u8 {
    ...
    try json.appendRawJsonArray(&out, allocator, pack.required_scopes_json);
    ...
    try json.appendRawJsonObject(&out, allocator, pack.sections_json);
    try out.appendSlice(allocator, ",\"citations\":");
    try json.appendRawJsonArray(&out, allocator, pack.citations_json);
    try out.appendSlice(allocator, ",\"forbidden_assumptions\":");
    try json.appendRawJsonArray(&out, allocator, pack.forbidden_assumptions_json);
    try out.appendSlice(allocator, ",\"suggested_next_steps\":");
    try json.appendRawJsonArray(&out, allocator, pack.suggested_next_steps_json);
    try out.appendSlice(allocator, ",\"included_sources\":");
    try json.appendRawJsonArray(&out, allocator, pack.included_sources_json);
    try out.appendSlice(allocator, ",\"included_artifacts\":");
    try json.appendRawJsonArray(&out, allocator, pack.included_artifacts_json);
    try out.appendSlice(allocator, ",\"included_memory_atoms\":");
    try json.appendRawJsonArray(&out, allocator, pack.included_memory_atoms_json);
    try out.appendSlice(allocator, ",\"included_result_refs\":");
    try json.appendRawJsonArray(&out, allocator, pack.included_result_refs_json);
...
test "feed contract enforces raw container root types" {
    const allocator = std.testing.allocator;
    const invalid_permissions_entry = domain.AgentMemory{
        .id = "mem_bad_raw",
        .key = "pref",
        .content = "value",
        .category = "core",
    ...
    try std.testing.expectError(error.InvalidRawJson, agentMemoryWrite(allocator, "native", invalid_permissions_entry, .put, "{}"));

    var entry = invalid_permissions_entry;
    entry.permissions_json = "[]";
    try std.testing.expectError(error.InvalidRawJson, agentMemoryWrite(allocator, "native", entry, .merge_string_set, "{\"value\":\"wrong-root\"}"));
    try std.testing.expectError(error.InvalidRawJson, agentMemoryWrite(allocator, "native", entry, .merge_object, "[\"wrong-root\"]"));

    const invalid_policy_permissions = .{
        .scope = "team:alpha",
        .visibility = "team",
        .permissions_json = "{\"scope\":\"team:alpha\"}",
        .owner = @as(?[]const u8, null),
        .ttl_ms = @as(?i64, null),
        .review_after_ms = @as(?i64, null),
        .metadata_json = "[\"wrong-root\"]",
    };
    try std.testing.expectError(error.InvalidRawJson, policyScope(allocator, invalid_policy_permissions));

    const invalid_policy_metadata = .{
        .scope = "team:alpha",
        .visibility = "team",
        .permissions_json = "[]",
        .owner = @as(?[]const u8, null),
    ...
    try std.testing.expectError(error.InvalidRawJson, policyScope(allocator, invalid_policy_metadata));
}
...
test "feed sequence helpers saturate frontier increments" {
    const max = std.math.maxInt(i64);
    try std.testing.expectEqual(@as(i64, 1), feedSequenceAfter(0));
    try std.testing.expectEqual(max, feedSequenceAfter(max));
    try std.testing.expectEqual(@as(i64, 42), feedNextLocalOriginSequence(42, 10));
    try std.testing.expectEqual(@as(i64, 11), feedNextLocalOriginSequence(0, 10));
    try std.testing.expectEqual(max, feedNextLocalOriginSequence(0, max));
    try std.testing.expectEqual(@as(i64, 21), feedNextLocalOriginSequenceFromFrontier(20, 10));
    try std.testing.expectEqual(@as(i64, 11), feedNextLocalOriginSequenceFromFrontier(3, 10));
    try std.testing.expectEqual(max, feedNextLocalOriginSequenceFromFrontier(max, 10));
    try std.testing.expectEqual(max, feedNextLocalOriginSequenceFromFrontier(3, max));
}

test "feed payload route injection uses canonical route fields" {
    const allocator = std.testing.allocator;

    const native = try withStorageRoute(allocator, "{\"title\":\"native\"}", TestRoute{ .target = .native });
    defer allocator.free(native);
    try std.testing.expect(std.mem.indexOf(u8, native, "\"store\":\"native\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, native, "\"storage\"") == null);

    const all = try withStorageRoute(allocator, "{\"title\":\"all\"}", TestRoute{ .target = .all });
    defer allocator.free(all);
    try std.testing.expect(std.mem.indexOf(u8, all, "\"store\":\"all\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, all, "\"storage\"") == null);
}
...
test "feed payload route override replaces explicit top-level selectors" {
    const allocator = std.testing.allocator;
    const payload = "{\"store\":\"archive\",\"title\":\"explicit\",\"metadata\":{\"store\":\"nested\"}}";
    const routed = try withStorageRouteOverride(allocator, payload, TestRoute{ .target = .named, .name = "scratch" });
    defer allocator.free(routed);

    try std.testing.expect(std.mem.indexOf(u8, routed, "\"store\":\"scratch\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, routed, "\"store\":\"archive\"") == null);
    try std.testing.expect(std.mem.indexOf(u8, routed, "\"metadata\":{\"store\":\"nested\"}") != null);
}
...
test "feed object and reference policies are centralized" {
    ...
    try std.testing.expect(isDeleteOperation("forget"));
    try std.testing.expect(isAgentMemoryDeleteOperation("delete_scoped"));
    try std.testing.expect(isAgentSessionDeleteOperation("delete_all"));
    try std.testing.expect(!isAgentSessionDeleteOperation("delete_scoped"));
...
test "feed operation inference and lifecycle status are centralized" {
    ...
    try std.testing.expectEqualStrings("forget", operationFromEventType("memory_atom.forget"));
    try std.testing.expectEqualStrings("forget", operationFromEventType("agent_memory.runtime_forget"));
    try std.testing.expectEqualStrings("delete", operationFromEventType("agent_memory_runtime_delete"));
    ...
    try std.testing.expectEqualStrings("verify", agentMemoryLifecycleOperationForStatus("verified"));
    try std.testing.expectEqualStrings("mark_stale", agentMemoryLifecycleOperationForStatus("stale"));
    try std.testing.expectEqualStrings("supersede", agentMemoryLifecycleOperationForStatus("superseded"));
    try std.testing.expectEqualStrings("forget", agentMemoryLifecycleOperationForStatus("deprecated"));
    try std.testing.expectEqualStrings("verify", agentMemoryLifecycleOperationForStatus("unknown"));
...
test "feed event permissions accept json-encoded string fields" {
    const allocator = std.testing.allocator;
    var parsed = try std.json.parseFromSlice(std.json.Value, allocator, "{\"permissions\":\"[\\\"public\\\",\\\"team:alpha\\\"]\"}", .{});
    defer parsed.deinit();
    const permissions = try eventPermissions(allocator, parsed.value.object, "agent_memory", "{}", "agent:a");
    defer allocator.free(permissions);
    try std.testing.expectEqualStrings("[\"public\",\"team:alpha\"]", permissions);

    var payload_parsed = try std.json.parseFromSlice(std.json.Value, allocator, "{}", .{});
    defer payload_parsed.deinit();
    const payload_permissions = try eventPermissions(allocator, payload_parsed.value.object, "agent_memory", "{\"permissions_json\":\"[\\\"team:beta\\\"]\"}", "agent:a");
    defer allocator.free(payload_permissions);
    try std.testing.expectEqualStrings("[\"team:beta\"]", payload_permissions);

    var invalid = try std.json.parseFromSlice(std.json.Value, allocator, "{\"permissions_json\":\"[\\\"team:broken\\\",]\"}", .{});
    defer invalid.deinit();
    try std.testing.expectError(error.InvalidRawJson, eventPermissions(allocator, invalid.value.object, "agent_memory", "{}", "agent:a"));

    try std.testing.expectError(error.InvalidRawJson, eventPermissions(allocator, payload_parsed.value.object, "agent_memory", "{\"permissions_json\":\"[\\\"team:broken\\\",]\"}", "agent:a"));

    var invalid_root = try std.json.parseFromSlice(std.json.Value, allocator, "{\"permissions\":{\"scope\":\"public\"}}", .{});
    defer invalid_root.deinit();
    try std.testing.expectError(error.InvalidRawJson, eventPermissions(allocator, invalid_root.value.object, "agent_memory", "{}", "agent:a"));
    try std.testing.expectError(error.InvalidRawJson, eventPermissions(allocator, payload_parsed.value.object, "agent_memory", "{\"permissions\":{\"scope\":\"public\"}}", "agent:a"));
}

test "feed event permissions skip null aliases before encoded JSON aliases" {
    const allocator = std.testing.allocator;
    var parsed = try std.json.parseFromSlice(std.json.Value, allocator, "{\"permissions\":null,\"permissions_json\":\"[\\\"team:alpha\\\"]\"}", .{});
    defer parsed.deinit();
    const permissions = try eventPermissions(allocator, parsed.value.object, "agent_memory", "{}", "agent:a");
    defer allocator.free(permissions);
    try std.testing.expectEqualStrings("[\"team:alpha\"]", permissions);

    var payload_parsed = try std.json.parseFromSlice(std.json.Value, allocator, "{}", .{});
    defer payload_parsed.deinit();
    const payload_permissions = try eventPermissions(allocator, payload_parsed.value.object, "agent_memory", "{\"permissions\":null,\"permissions_json\":\"[\\\"team:beta\\\"]\"}", "agent:a");
    defer allocator.free(payload_permissions);
    try std.testing.expectEqualStrings("[\"team:beta\"]", payload_permissions);

    var invalid = try std.json.parseFromSlice(std.json.Value, allocator, "{\"permissions\":null,\"permissions_json\":{\"scope\":\"public\"}}", .{});
    defer invalid.deinit();
    try std.testing.expectError(error.InvalidRawJson, eventPermissions(allocator, invalid.value.object, "agent_memory", "{}", "agent:a"));
    try std.testing.expectError(error.InvalidRawJson, eventPermissions(allocator, payload_parsed.value.object, "agent_memory", "{\"permissions\":null,\"permissions_json\":{\"scope\":\"public\"}}", "agent:a"));
}
...
test "feed wire metadata helpers are centralized" {
    ...
    try std.testing.expectEqualStrings("verify", wireOperation("agent_memory", "verify", true));
    ...
    try std.testing.expect(nullClawMemoryOperationSupported("put"));
    try std.testing.expect(nullClawMemoryOperationSupported("delete_scoped"));
    try std.testing.expect(nullClawMemoryOperationSupported("delete_all"));
    try std.testing.expect(!nullClawMemoryOperationSupported("delete"));
    try std.testing.expect(!nullClawMemoryOperationSupported("forget"));
    try std.testing.expect(!nullClawMemoryOperationSupported("verify"));
    try std.testing.expect(!nullClawMemoryOperationSupported("mark_stale"));
    try std.testing.expect(!nullClawMemoryOperationSupported("supersede"));
    try std.testing.expect(nullClawMemoryApplyOperationSupported("forget"));
    try std.testing.expect(nullClawMemoryApplyOperationSupported("verify"));
    try std.testing.expect(nullClawMemoryApplyOperationSupported("mark_stale"));
    try std.testing.expect(nullClawMemoryApplyOperationSupported("supersede"));
}
