...
pub const max_snippet_chars_limit: usize = 256 * 1024;
pub const max_injected_chars_limit: usize = 1024 * 1024;
...
fn applyLimitFields(limits: *Limits, obj: std.json.ObjectMap) void {
    if (json.intField(obj, "max_results")) |value| limits.max_results = positiveUsize(value, limits.max_results, max_results_limit);
    if (json.intField(obj, "max_snippet_chars")) |value| limits.max_snippet_chars = positiveUsize(value, limits.max_snippet_chars, max_snippet_chars_limit);
    if (json.intField(obj, "max_injected_chars")) |value| limits.max_injected_chars = positiveUsize(value, limits.max_injected_chars, max_injected_chars_limit);
}

fn positiveUsize(value: i64, fallback: usize, max_value: usize) usize {
...
fn normalizeObject(
    ...
    const bounded_content = nextBoundedContentSlice(content, limits, total_chars) orelse return null;

    const metadata_json = try metadataJson(allocator, obj, path, title, bounded_content, rank);
    errdefer allocator.free(metadata_json);

    return .{
        .title = try allocator.dupe(u8, title),
        .content = try allocator.dupe(u8, bounded_content),
        .raw_content_uri = if (path) |p| try allocator.dupe(u8, p) else null,
        .checksum = try checksum(allocator, path, bounded_content),
        .metadata_json = metadata_json,
    };
}

fn nextBoundedContentSlice(content: []const u8, limits: Limits, total_chars: *usize) ?[]const u8 {
    if (total_chars.* >= limits.max_injected_chars) return null;
    const remaining = limits.max_injected_chars - total_chars.*;
    const clipped_len = @min(@min(content.len, limits.max_snippet_chars), remaining);
    total_chars.* += clipped_len;
    return content[0..clipped_len];
}
...
pub fn pruneSessionExports(directory: []const u8, retention_days: u32) !SessionPruneResult {
    var dir = try std.Io.Dir.cwd().openDir(compat.io(), directory, .{ .iterate = true, .follow_symlinks = false });
    ...
    while (try iter.next(compat.io())) |entry| {
        ...
        const mtime_ms = ids.timestampNanosecondsToMs(stat.mtime.nanoseconds);
        if (!sessionExportExpired(now_ms, mtime_ms, retention_days)) {
            result.skipped += 1;
            continue;
        }
...
fn sessionExportExpired(now_ms: i64, mtime_ms: i64, retention_days: u32) bool {
    if (retention_days == 0) return true;
    return time_math.elapsedSinceMs(now_ms, mtime_ms) >= retentionDaysMs(retention_days);
}

fn retentionDaysMs(retention_days: u32) i64 {
    const raw = @as(u64, retention_days) * std.time.ms_per_day;
    return std.math.cast(i64, raw) orelse std.math.maxInt(i64);
}

test "qmd session export retention expiration uses bounded age math" {
    try std.testing.expect(sessionExportExpired(1_000, 1_000, 0));
    try std.testing.expect(!sessionExportExpired(1_000, 1_000, 1));
    try std.testing.expect(!sessionExportExpired(1_000, 2_000, 1));
    try std.testing.expect(sessionExportExpired(std.time.ms_per_day, 0, 1));
    try std.testing.expect(sessionExportExpired(std.math.maxInt(i64), std.math.minInt(i64), std.math.maxInt(u32)));
}
...
test "qmd adapter preserves and rejects metadata raw object roots" {
    const valid_body =
        \\{"path":"docs/valid.md","content":"Decision: preserve metadata","metadata":{"source":"qmd","rank":1}}
    ;
    var valid = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, valid_body, .{});
    defer valid.deinit();
    const items = try normalizeValue(std.testing.allocator, valid.value, .{});
    defer deinitItems(std.testing.allocator, items);

    try std.testing.expectEqual(@as(usize, 1), items.len);
    try std.testing.expect(std.mem.indexOf(u8, items[0].metadata_json, "\"metadata\":{\"source\":\"qmd\",\"rank\":1}") != null);

    const invalid_body =
        \\{"path":"docs/bad.md","content":"Decision: reject bad metadata","metadata":["not-object"]}
    ;
    var invalid = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, invalid_body, .{});
    defer invalid.deinit();
    try std.testing.expectError(error.InvalidRawJson, normalizeValue(std.testing.allocator, invalid.value, .{}));
}

test "qmd adapter stores bounded content instead of raw payloads" {
    const body =
        \\[
        \\ {"path":"docs/a.md","content":"abcdefghij"},
        \\ {"path":"docs/b.md","content":"klmnopqrst"}
        \\]
    ;
    const parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, body, .{});
    defer parsed.deinit();
    const items = try normalizeValue(std.testing.allocator, parsed.value, .{ .max_results = 4, .max_snippet_chars = 5, .max_injected_chars = 8 });
    defer deinitItems(std.testing.allocator, items);

    try std.testing.expectEqual(@as(usize, 2), items.len);
    try std.testing.expectEqualStrings("abcde", items[0].content);
    try std.testing.expectEqualStrings("klm", items[1].content);
    try std.testing.expect(std.mem.indexOf(u8, items[0].metadata_json, "fghij") == null);

    const expected_checksum = try checksum(std.testing.allocator, "docs/a.md", "abcde");
    defer std.testing.allocator.free(expected_checksum);
    try std.testing.expectEqualStrings(expected_checksum, items[0].checksum);
}

test "qmd adapter clamps attacker supplied limits" {
    const body =
        \\{
        \\  "max_results": -1,
        \\  "limits": {
        \\    "max_results": 9223372036854775807,
        \\    "max_snippet_chars": 9223372036854775807,
        \\    "max_injected_chars": 9223372036854775807
        \\  }
        \\}
    ;
    const parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, body, .{});
    ...
    const limits = limitsFromObject(parsed.value.object);
    try std.testing.expectEqual(max_results_limit, limits.max_results);
    try std.testing.expectEqual(max_snippet_chars_limit, limits.max_snippet_chars);
    try std.testing.expectEqual(max_injected_chars_limit, limits.max_injected_chars);

    const negative = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, "{\"limits\":{\"max_results\":-9,\"max_snippet_chars\":-8,\"max_injected_chars\":-7}}", .{});
    defer negative.deinit();
    const negative_limits = limitsFromObject(negative.value.object);
    try std.testing.expectEqual(@as(usize, 6), negative_limits.max_results);
    try std.testing.expectEqual(@as(usize, 700), negative_limits.max_snippet_chars);
    try std.testing.expectEqual(@as(usize, 4000), negative_limits.max_injected_chars);
}
