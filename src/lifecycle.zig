...
const cache_time = @import("cache_time.zig");
...
pub const CacheEntry = struct {
    ...
    pub fn isFresh(self: CacheEntry, now_ms: i64) bool {
        return cache_time.isFresh(self.created_at_ms, self.ttl_ms, now_ms);
    }
};
...
pub fn hygieneDecision(status: []const u8, last_verified_at_ms: ?i64, now_ms: i64, stale_after_ms: i64, archive_after_ms: i64, purge_after_ms: i64) HygieneDecision {
    ...
    const age = time_math.elapsedSinceMs(now_ms, last_seen);
...
pub fn memoryStalenessNeedsReview(stale_memory_atoms: usize, total_memory_atoms: usize) bool {
    return total_memory_atoms > 0 and stale_memory_atoms > total_memory_atoms / 2;
}
...
pub const RolloutMode = enum {
    ...
    pub fn parse(value: ?[]const u8, has_percent: bool) !RolloutMode {
        ...
        return error.InvalidRolloutMode;
    }
...
pub const RolloutPolicy = struct {
    ...
    pub fn validateUsable(self: RolloutPolicy) !void {
        if (self.percent > 100) return error.InvalidRolloutPolicy;
        if (self.shadow_percent > 100) return error.InvalidRolloutPolicy;
        try validateStringArrayJson(self.required_scopes_json);
        try validateStringArrayJson(self.blocked_scopes_json);
        try validateStringArrayJson(self.target_scopes_json);
        try validateStringArrayJson(self.required_capabilities_json);
        try validateStringArrayJson(self.blocked_capabilities_json);
    }
};
...
fn validateStringArrayJson(raw: []const u8) !void {
    if (!domain.jsonStringArrayItemsNonBlank(raw)) return error.InvalidRolloutPolicy;
}
...
pub const Diagnostics = struct {
    ...
    vector_outbox_running: usize = 0,
    vector_outbox_failed: usize = 0,
    vector_outbox_expired_running: usize = 0,
    lucid_projection_pending: usize = 0,
    lucid_projection_failed: usize = 0,
    graph_projection_pending: usize = 0,
    graph_projection_failed: usize = 0,
    cache_entries: usize,
    response_cache_entries: usize = 0,
    semantic_cache_entries: usize = 0,
    embedding_cache_entries: usize = 0,
    expired_response_cache_entries: usize = 0,
    expired_semantic_cache_entries: usize = 0,
    queued_jobs: usize = 0,
    running_jobs: usize = 0,
    failed_jobs: usize = 0,
    expired_running_jobs: usize = 0,
    pending_feed_events: usize = 0,
    applying_feed_events: usize = 0,
    ...
    pub fn health(self: Diagnostics) []const u8 {
        ...
        if (self.expired_running_jobs > 0) return "degraded";
        if (self.lucid_projection_failed > 0) return "degraded";
        if (self.graph_projection_failed > 0) return "degraded";
        if (self.vector_outbox_failed > 0) return "degraded";
        if (self.vector_outbox_expired_running > 0) return "degraded";
        if (self.vector_outbox_pending > 1000) return "degraded";
        if (self.lucid_projection_pending > 1000) return "degraded";
        if (self.graph_projection_pending > 1000) return "degraded";
        if (self.queued_jobs > 1000 or self.pending_feed_events > 1000 or self.applying_feed_events > 1000) return "degraded";
        if (memoryStalenessNeedsReview(self.stale_memory_atoms, self.total_memory_atoms)) return "needs_review";
        return "ok";
    }
};
...
test "lifecycle cache freshness respects ttl" {
    ...
    const overflow_entry = CacheEntry{ .key = "q", .value = "a", .created_at_ms = std.math.maxInt(i64) - 10, .ttl_ms = 100 };
    try std.testing.expect(overflow_entry.isFresh(std.math.maxInt(i64) - 1));
}
...
test "lifecycle memory staleness threshold is overflow safe" {
    try std.testing.expect(!memoryStalenessNeedsReview(1, 0));
    try std.testing.expect(!memoryStalenessNeedsReview(5, 10));
    try std.testing.expect(memoryStalenessNeedsReview(6, 10));
    try std.testing.expect(memoryStalenessNeedsReview(2, 3));
    try std.testing.expect(memoryStalenessNeedsReview(std.math.maxInt(usize), std.math.maxInt(usize)));

    const healthy = Diagnostics{
        .total_memory_atoms = 10,
        .stale_memory_atoms = 5,
        .vector_outbox_pending = 0,
        .cache_entries = 0,
    };
    try std.testing.expectEqualStrings("ok", healthy.health());

    const stale = Diagnostics{
        .total_memory_atoms = std.math.maxInt(usize),
        .stale_memory_atoms = std.math.maxInt(usize),
        .vector_outbox_pending = 0,
        .cache_entries = 0,
    };
    try std.testing.expectEqualStrings("needs_review", stale.health());
}
...
test "lifecycle hygiene age is overflow safe" {
    try std.testing.expectEqual(
        HygieneDecision.purge,
        hygieneDecision("verified", std.math.minInt(i64), std.math.maxInt(i64), 1, 2, std.math.maxInt(i64) - 1),
    );
    try std.testing.expectEqual(
        HygieneDecision.keep,
        hygieneDecision("verified", std.math.maxInt(i64), 0, 1, 2, 3),
    );
}
...
test "lifecycle rollout mode parser fails closed on unknown names" {
    try std.testing.expectEqual(RolloutMode.off, try RolloutMode.parse(null, false));
    try std.testing.expectEqual(RolloutMode.canary, try RolloutMode.parse(null, true));
    try std.testing.expectEqual(RolloutMode.off, try RolloutMode.parse("off", false));
    try std.testing.expectEqual(RolloutMode.shadow, try RolloutMode.parse("shadow", false));
    try std.testing.expectEqual(RolloutMode.canary, try RolloutMode.parse("canary", false));
    try std.testing.expectEqual(RolloutMode.on, try RolloutMode.parse("on", false));
    try std.testing.expectError(error.InvalidRolloutMode, RolloutMode.parse("canray", false));
}

test "lifecycle rollout policy validates scope and capability lists" {
    try (RolloutPolicy{
        .mode = .on,
        .required_scopes_json = "[\"project:nullpantry\"]",
        .blocked_scopes_json = "[]",
        .target_scopes_json = "[\"team:agents\"]",
        .required_capabilities_json = "[\"read\"]",
        .blocked_capabilities_json = "[\"export\"]",
    }).validateUsable();

    try std.testing.expectError(error.InvalidRolloutPolicy, (RolloutPolicy{ .required_scopes_json = "\"project:nullpantry\"" }).validateUsable());
    try std.testing.expectError(error.InvalidRolloutPolicy, (RolloutPolicy{ .required_scopes_json = "[\"project:nullpantry\"" }).validateUsable());
    try std.testing.expectError(error.InvalidRolloutPolicy, (RolloutPolicy{ .required_scopes_json = "[1]" }).validateUsable());
    try std.testing.expectError(error.InvalidRolloutPolicy, (RolloutPolicy{ .required_scopes_json = "[\"\"]" }).validateUsable());
    try std.testing.expectError(error.InvalidRolloutPolicy, (RolloutPolicy{ .required_capabilities_json = "[\"read\",\"  \"]" }).validateUsable());
}
